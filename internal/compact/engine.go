...
func Run(cfg Config) error {
	...
	if old, err := state.Load(statePath); err == nil {
		if old.Source == cfg.Source && old.Output == cfg.Output && old.Inode == info.Inode && old.Device == info.Device && old.CutoffOffset == cutoff && old.Gzip == cfg.GzipOutput {
			jobState = old
			fmt.Println("Resuming from state offset:", human.FormatBytes(jobState.LastPunchedOffset))
		} else {
	...
	for offset < cutoff {
		chunkNo++
		freeNow, err := disk.FreeBytes(outDir)
		if err != nil {
			return err
		}
		chunkSize := chooseChunkSize(freeNow, cfg.WorkingPercent, lastRatio, cfg.MinChunk, cfg.MaxChunk)
		if chunkSize <= 0 {
			return fmt.Errorf("free space too low for next safe chunk; current free=%s", human.FormatBytes(freeNow))
		}
		...
		fmt.Printf("chunk=%d raw=%s archived=%s punched=%s next_offset=%s free=%s ratio=%.2f%% time=%s\n", chunkNo, human.FormatBytes(raw), human.FormatBytes(written), human.FormatBytes(raw), human.FormatBytes(offset), human.FormatBytes(freeAfter), lastRatio*100, time.Since(started).Round(time.Millisecond))
	}
...
func appendData(outputPath string, data []byte, gzipEnabled bool) error {
	...
	if gzipEnabled {
		...
		if _, err := out.Write(data); err != nil { _ = out.Close(); return err }
	}
